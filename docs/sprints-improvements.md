...
| 18 | S18 | C1 Animated progress bars | Replace static progress text with animated `QProgressBar` per endpoint smoothly animating to target | Progress bars animate to target; per-endpoint | Qt widget test: animation reaches target value | P8 (dashboard) | M | ASSIGNED |
